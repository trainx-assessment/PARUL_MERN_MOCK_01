...
try {
  ...
  console.error("Failed to load stored student data:", error);
  students = [];
}
...
function validateEmail() {
  ...
  if (isDuplicateEmail(value, editId)) {
...
function validatePhone() {
  ...
  if (isDuplicatePhone(value, editId)) {
    showError("phone", "A student with this phone number already exists.");
    return false;
  }
  clearError("phone");
  return true;
...
function saveToStorage() {
  try {
    ...
    alert(
      "Warning: Could not save data to local storage. Your changes may be lost on refresh.",
    );
  }
}
